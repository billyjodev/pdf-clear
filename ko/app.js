...
class PdfRegionEraser {
    ...
    getColorModeName(mode) {
        const names = { dominant: '대표색', border: '테두리색', custom: '사용자 지정' };
    ...
    async handleFileSelect(event) {
        ...
        try {
            ...
            this.showToast('PDF 파일을 불러오는 중 오류가 발생했습니다.');
        }
    }
    ...
    async handleRightClickDelete(x1, y1, x2, y2) {
        ...
        if (selectionsToDelete.length === 0) {
            this.showToast('이 영역에 삭제할 선택사항이 없습니다');
            return;
        }

        const message = `${selectionsToDelete.length}개 선택사항을 삭제하시겠습니까?`;
        ...
        this.refreshUI();
        this.showToast(`${selectionsToDelete.length}개 선택사항이 삭제되었습니다.`);
    }
    ...
    updateSelectionInfo() {
        ...
        this.selectionCountSpan.textContent = `적용됨: ${appliedCount}, 대기중: ${pendingCount}`;
    }
    ...
    updateSelectionList() {
        ...
        pageHeader.innerHTML = `
            <span class="font-medium text-gray-700 text-sm">${this.currentPage} 페이지</span>
            <div class="flex gap-1">
                ${pendingSelections.length > 0 ? `<button class="text-green-600 hover:text-green-800 text-xs px-1.5 py-0.5 border border-green-300 rounded hover:bg-green-50" onclick="app.applyAllOnPage(${this.currentPage - 1})">적용</button>` : ''}
                ${selections.length > 0 ? `<button class="text-yellow-600 hover:text-yellow-800 text-xs px-1.5 py-0.5 border border-yellow-300 rounded hover:bg-yellow-50" onclick="app.revertAllOnPage(${this.currentPage - 1})">되돌리기</button>` : ''}
                ${(pendingSelections.length + selections.length) > 0 ? `<button class="text-red-600 hover:text-red-800 text-xs px-1.5 py-0.5 border border-red-300 rounded hover:bg-red-50" onclick="app.clearAllOnPage(${this.currentPage - 1})">삭제</button>` : ''}
            </div>
        `;
        ...
        item.innerHTML = `
            <div class="flex items-center justify-between w-full">
                ...
                <div class="flex gap-0.5 ml-auto">
                    <button class="text-green-500 hover:text-green-700 text-xs px-1 py-0.5" onclick="app.applySelection(${pi}, ${index})">적용</button>
                    <button class="text-red-500 hover:text-red-700 text-xs px-1 py-0.5" onclick="app.removePendingSelection(${pi}, ${index})">삭제</button>
                </div>
            </div>
            <div class="flex items-center gap-1 w-full">
                <select class="text-xs border rounded px-1 py-0.5 bg-white" onchange="app.updateSelectionScope(${pi}, ${index}, this.value)">
                    <option value="current" ${sel.applyScope === 'current' ? 'selected' : ''}>현재</option>
                    <option value="all" ${sel.applyScope === 'all' ? 'selected' : ''}>전체</option>
                    <option value="range" ${sel.applyScope === 'range' ? 'selected' : ''}>범위</option>
                </select>
    ...
    renderAppliedItem(item, sel, pi, index) {
        ...
        if (sel.groupId) {
            ...
            const rangeText = sel.applyScope === 'all' ? '전체' : groupPages.join(', ');
            groupLabel = ` <span class="text-blue-500 cursor-default" title="페이지: ${rangeText}">(G)</span>`;
            groupTooltip = `페이지: ${rangeText}`;
        }
        const isSourcePage = !sel.groupId || sel.sourcePage === this.currentPage;
        const revertBtn = isSourcePage
            ? `<button class="text-yellow-600 hover:text-yellow-800 text-xs px-1 py-0.5" onclick="app.toggleSelection(${pi}, ${index})">되돌리기</button>`
            : '';
        ...
        item.innerHTML = `
            ...
            <div class="flex gap-0.5 ml-auto">
                ${revertBtn}
                <button class="text-red-500 hover:text-red-700 text-xs px-1 py-0.5" onclick="app.removeAppliedSelection(${pi}, ${index})">삭제</button>
            </div>
        `;
    }
    ...
    updateButtons() {
        ...
        if (hasAnyPending) {
            this.applyAllToggleBtn.textContent = '모두 적용';
            ...
            this.applyAllToggleBtn.textContent = '모두 되돌리기';
    ...
    applyAllPending() {
        ...
        if (applied) {
            this.renderCurrentPage();
            this.showToast('모든 선택사항이 적용되었습니다.');
        }
    }

    async revertAllApplied() {
        if (!this.pagesData.some(p => p.selections.length > 0)) return;
        if (!await this.showConfirm('모든 적용된 선택사항을 되돌리시겠습니까?')) return;
        ...
        this.renderCurrentPage();
        this.showToast('모든 선택사항이 되돌려졌습니다.');
    }

    async clearAllSelections() {
        if (!await this.showConfirm('모든 선택사항을 지우시겠습니까?')) return;
    ...
    applyAllOnPage(pageIndex) {
        ...
        this.renderPage(pageData.pageNumber);
        this.refreshUI();
        this.showToast(`페이지 ${pageIndex + 1}: 모든 선택사항이 적용되었습니다.`);
    }
    ...
    async revertAllOnPage(pageIndex) {
        ...
        this.renderPage(pageData.pageNumber);
        this.refreshUI();
        this.showToast(`페이지 ${pageIndex + 1}: 모든 선택사항이 되돌려졌습니다.`);
    }
    ...
    async clearAllOnPage(pageIndex) {
        ...
        this.renderPage(pageData.pageNumber);
        this.refreshUI();
        this.showToast(`페이지 ${pageIndex + 1}: 모든 선택사항이 삭제되었습니다.`);
    }
    ...
    async luckySelect() {
        ...
        this.renderCurrentPage();
        this.refreshUI();
        this.showToast('🍀 럭키! 모든 페이지에 선택사항이 적용되었습니다.');
    }
    ...
    async downloadModifiedPdf() {
        ...
        try {
            ...
            this.showToast('PDF를 생성하는 중 오류가 발생했습니다.');
        }
    }
    ...
    async downloadAsPpt() {
        ...
        try {
            this.showProgress('PPT 생성 중...', '라이브러리 불러오는 중...', 5);
            ...
            for (let i = 1; i <= this.totalPages; i++) {
                ...
                this.updateProgress(`페이지 렌더링 중 ${i}/${this.totalPages}...`, progressPercent);
            ...
            this.updateProgress('슬라이드 생성 중...', 70);
            ...
            slideDataList.forEach(({ bgImage, croppedImages }, idx) => {
                ...
                this.updateProgress(`슬라이드 생성 중 ${idx + 1}/${slideDataList.length}...`, progressPercent);
            ...
            this.updateProgress('파일 쓰는 중...', 95);
            ...
            this.hideProgress();
            this.showToast('PPT를 생성하는 중 오류가 발생했습니다: ' + error.message);
        }
    }
    ...
    async autoDetectText() {
        ...
        try {
            ...
            for (let i = 1; i <= this.totalPages; i++) {
                this.autoDetectTextBtn.textContent = `감지 중... (${i}/${this.totalPages})`;
            ...
            this.refreshUI();
            this.showToast(`${this.totalPages}개 페이지에서 ${totalAdded}개 텍스트 영역이 감지되었습니다.`);
        } catch (error) {
            console.error('Text detection error:', error);
            this.showToast('텍스트 감지 중 오류가 발생했습니다: ' + error.message);
        } finally {
            this.autoDetectTextBtn.disabled = false;
            this.autoDetectTextBtn.textContent = '텍스트 자동 감지';
        }
    }
